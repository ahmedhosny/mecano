...
export class Graph {
  ...
  setRootNode() {
    let _this = this;
    const _root = this.nodes.filter(
      // TODO : only accepts single start point for now
      (obj) => obj.key === _this.root[0]
    );
  ...
  getChildren(node) {
    ...
    nextEdges.forEach((l, index) => {
      ...
      const nextNodes = this.nodes.filter((obj) => obj.key === l.target);
      if (nextNodes.length !== 0) {
        out.push(nextNodes[0]);
      }
    });
    return out;
  }
